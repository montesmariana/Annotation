...
function toggleVariable(variable) {
  const checkAttributes =
    d3.keys(personalizedVariables[variable]).indexOf("hasAttributes") !== -1;
...
function toggleSubvariable(subvariable) {
  ...
  variables.forEach(function (d) {
    if (d.type !== "default") {
      inputOptions[d.code] = d.label;
    }
  });
  inputOptions[msg["overall"]] =
    msg["overall"][0].toUpperCase() + msg["overall"].substr(1).toLowerCase();
  Swal.fire({
    ...
    if (result.value) {
      if (result.value === "overall") {
        ...
        const varAttributes =
          personalizedVariables[result.value]["hasAttributes"];
        if (varAttributes.indexOf(subvariable) === -1) {
          varAttributes.push(subvariable);
          if (selectedVariables.indexOf(result.value) !== -1)
            showAnnotations(subvariable, result.value);
        } else {
          varAttributes.splice(varAttributes.indexOf(subvariable), 1);
          if (selectedVariables.indexOf(result.value) !== -1)
            removeAnnotations(subvariable + "_" + result.value);
        }
      }
    }
  });
...
function createCategorical() {
  ...
  const selectTitle = d3
    .select("#catVariableBody")
    .append("form")
    .append("div")
  ...
  if (typesToAnnotate.length === 0) {
    ...
    const typeDivs = forms
      .selectAll("div.addCat")
      .data(typesToAnnotate)
      .enter()
      .append("div")
      .attr("class", "addCat");

    typeDivs.append("h5").text(function (d) {
      return msg["for_type"] + d;
    });
  ...
  Swal.fire({
    ...
    if (result.value) {
      ...
      if (d3.keys(fileContents).indexOf("hasAttributes") > -1) {
        ...
        showAnnotations(result.value);
        contents["hasAttributes"].forEach(function (x) {
          return showAnnotations(d, result.value);
        });
      } else {
        const contents = {
          type: "categorical",
...
function addPersonalizedVariable(name, content, path) {
  ...
  config["personalized_variables"].push({
    ...
    path: path,
    hasAttributes:
      d3.keys(content).indexOf("hasAttributes") === -1
        ? []
        : content["hasAttributes"],
  });
  variables.push({
    code: name,
    label: name[0].toUpperCase() + name.substr(1).toLowerCase(),
...
function showConc() {
  ...
  overview
    ...
    .enter()
    .append("div")
    .attr("class", "row no-gutters justify-content-sm-center")
    .each(function (d) {
  ...
  conc = d3
    .select("#" + msg["tab_2"])
    .append("div")
    .attr("class", "row justify-content-sm-center mt-5")
    .append("div")
    ...
    .enter()
    .append("div")
    .attr("token_id", function (d) {
      return d.id;
    }); // The 'token_id' attribute makes it available for nested elements

  displayConc(); // displays the concordance that corresponds to the current token

  /// FIRST show concordance

  conc.append("p").style("color", "#696969").html(msg["before_concordance"]);
  ...
  buttons = d3
    ...
    .append("div")
    .attr("class", "col-sm-auto")
    .append("div")
    .attr("class", "btn-group mt-2");

  buttons
    .append("button") // PREVIOUS
    .attr("type", "button")
  ...
  if (conc === undefined) {
    ...
    switch (variable) {
      ...
      case "comments":
        ...
        break;
      default:
        personalizedVariables[variable]["type"] === "numerical"
          ? addNumerical(variable)
          : addCategorical(variable);
    }
  }
}
...
function removeAnnotations(variable) {
  ...
  selectedVariables.forEach(function (d) {
    ...
    d3.selectAll(".instNum_" + type + "_" + d).text(
    ...
    if (d3.keys(personalizedVariables[d]).indexOf("hasAttributes") > -1) {
      personalizedVariables[d]["hasAttributes"].forEach(function (a) {
        const subInstNumber =
          instructionNumber.toString() +
          "." +
          (personalizedVariables[d]["hasAttributes"].indexOf(a) + 1).toString();
        d3.selectAll(".instNum_" + type + "_" + a + "_" + d).text(
          subInstNumber + "."
        );
      });
    }
...
function writeInstruction(text, variable) {
  ...
  const instructionNumberString =
    varNameSplit.length === 1
      ? instructionNumber.toString()
      : instructionNumber.toString() +
        "." +
        (
          personalizedVariables[varName]["hasAttributes"].indexOf(
            varNameSplit[0]
          ) + 1
        ).toString();
  d3.selectAll("." + type + "_" + variable)
    .append("p")
    .style("color", "#696969")
    .html(
      "<span class='instNum_" +
...
function addCategorical(variable) {
  ...
  block
    ...
    .classed("active", function (d) {
      var here = d3
        .select(this.parentNode.parentNode.parentNode.parentNode.parentNode)
        .attr("token_id");
      // var chosen = hasSense(here, type) && text[type][here][variable] === d.code;
      var chosen =
        d3.keys(text[type][here]).indexOf(variable) !== -1 &&
        text[type][here][variable] === d.code;
...
function addConfidence(supravariable = null) {
  const varName =
    supravariable === null ? "confidence" : "confidence_" + supravariable;
  ...
  conf.append("span").attr("class", "px-2").text(msg["confidence_none"]);
  ...
  conf.append("span").attr("class", "px-2").text(msg["confidence_all"]);

  // Control when confidence changes
  $(document).on(
    "change",
    'input[name="' + type + "_" + varName + '"]',
    function (event) {
      var analized = d3
        .select(
          this.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode
...
function addCues(supravariable = null) {
  ...
  cues = block
    ...
    .each(function (d) {
      ...
      line
        .append("span")
        .attr("class", "btn p-0 text-primary")
        .style("font-weight", "bold")
        .text(function (d) {
          return d.target;
        });
        ...
        line
          ...
          .classed("active", function (d) {
            ...
            var chosen =
              ...
              text[type][here][varName].indexOf(d.index) !== -1;
            return chosen ? true : null;
          })
          .text(function (d) {
            return d.cw;
  ...
  $("." + type + "_" + varName).on(
    "change",
    "input[name='" + type + counter + "_" + varName + "']",
    function () {
      var analized = d3
        .select(
          this.parentNode.parentNode.parentNode.parentNode.parentNode.parentNode
...
function addComments() {
  ...
  inputtext
    ...
    .attr("class", "form-control")
    .attr("id", function (d) {
      return type + toClassify.indexOf(d.id) + "_comments";
    })
    .attr("name", "comments")
    ...
    .property("value", function () {
      ...
      return d3.keys(text[type][here]).indexOf("comments") !== -1
        ? text[type][here]["comments"]
        : null;
    });
...
function displayReminder() {
  reminder.style("display", function (d) {
    ...
    var here = d3
      ...
      .attr("token_id");
    var needCues =
      d3.keys(text[type]).indexOf(here) === -1 ||
...
function ableAnns() {
  ...
  cues.selectAll("label").classed("active", function (d) {
    var here = d3
      ...
      .attr("token_id");
    var chosen =
      hasSense(here, type) && text[type][here]["cues"].indexOf(d.index) > -1;
    return chosen ? true : null;
  });

  inputtext.selectAll("input").attr("disabled", function () {
    var here = d3
      .select(this.parentNode.parentNode.parentNode)
      .attr("token_id");
    return hasSense(here, type) ? null : true;
...
function displayNoSense() {
  no_sense_message.style("display", function (d) {
    ...
    return hasSense(here, type) &&
      ...
      d3.keys(text[type][here]).indexOf("comments") == -1
      ? "block"
      : "none";
  });
}
...
function offerTypes(selectedTypes) {
  ...
  typesel
    .append("div")
    .attr("class", "btn-group-vertical btn-group-toggle")
    .attr("data-toggle", "buttons")
    .selectAll("label")
    .data(selectedTypes)
    .enter()
    ...
    .classed("active", (d) => selectedTypes.indexOf(d) === 0)
    .html((d) => d)
    .append("input")
    .attr("type", "radio")
    .attr("autocomplete", "off")
  ...
  Swal.fire({
    ...
    if (value.value === "change") {
      ...
      config["types"].forEach(function (t) {
        types[t["name"]] = d3.tsvParse(
          fs.readFileSync(t["path"], (options = { encoding: "utf-8" }))
        );
      });
      config["personalized_variables"].forEach(function (v) {
        ...
        if (v["path"] === "numerical") {
          ...
          const hasAttributes =
            d3.keys(content).indexOf("hasAttributes") === -1
              ...
              : content["hasAttributes"];
          const values =
            d3.keys(content).indexOf("values") === -1
              ? content
              : content["values"];
          personalizedVariables[v["variable"]] = {
            type: "categorical",
        ...
        variables.push({
          code: v["variable"],
          label:
            v["variable"][0].toUpperCase() +
            v["variable"].substr(1).toLowerCase(),
          type: varType,
        });
      });
...
function checkAchievements(t) {
  ...
  const startedTokens = goal.filter(function (d) {
    return hasSense(d, t);
  });
  const doneTokens = goal.filter(function (d) {
    return hasSense(d, t) && isDone(d, t);
  });

  if (startedTokens.length == Math.ceil((goal.length * 3) / 5)) {
    return "halfdone";
  } else if (
    startedTokens.length == goal.length &&
    doneTokens.length === goal.length
  ) {
    return "done";
  } else {
    return "notdone";
...
function announceAchievement() {
  ...
  if (status == "done" && announced[type] != "done") {
    ...
    if (full_types.length == selectedTypes.length) {
      ...
      message =
        msg["type_done_message"][0] + type + msg["type_done_message"][1];
      timer = 1000;
    }
    ...
    message =
      msg["almost_done_message"][0] + type + msg["almost_done_message"][1];
    timer = 1000;
  } else {
    return;
  }
...
function createTsv(text, types) {
  const annotations = d3.keys(text).filter(function (d) {
    return typeof text[d] === "object";
  });
  const varNames = [...selectedVariables];
  d3.keys(personalizedVariables).forEach((v) => {
    if (
      selectedVariables.indexOf(v) > -1 &&
      personalizedVariables[v]["hasAttributes"].length > 0
    ) {
      ...
      return d3
        ...
        .map(function (t) {
          ...
          varNames.forEach(function (c) {
            // c is a variable
            res.push(
              d3.keys(text[a][t]).indexOf(c) === -1 ? "" : text[a][t][c]
            );
          });
          return res.join("\t");
        })
...
function saveInLS() {
  // check if storage is available and store
